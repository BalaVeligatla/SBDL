...
def get_account_schema():
    schema = """load_date date, active_ind int, account_id string,
                source_sys string, account_start_date timestamp, legal_title_1 string,
                legal_title_2 string, tax_id_type string, tax_id string, branch_code string, 
                country string"""
    return schema


def get_party_schema():
    schema = """load_date string,account_id string,party_id string,
            relation_type string,relation_start_date string"""
    return schema
...
def read_parties(spark, env, enable_hive, hive_db):
    ...
    if enable_hive:
        ...
        df = df.withColumn("load_date", to_date("load_date", "M/d/yyyy")) \
               .withColumn("relation_start_date", to_timestamp("relation_start_date"))
    return df
